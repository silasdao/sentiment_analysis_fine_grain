...
class MultiHeadAttention(object):
    """ multi head attention"""
    def __init__(self,Q,K_s,V_s,d_model,d_k,d_v,sequence_length,h,typee=None,is_training=None,mask=None,dropout_rate=0.1):
    ...
    def multi_head_attention_fn(self):
        ...
        # 2. scaled dot product attention for each projected version of Q,K,V
        dot_product=self.scaled_dot_product_attention_batch(Q_projected,K_s_projected,V_s_projected) # [batch,h,sequence_length,d_v]
        # 3. concatenated
    ...
    def scaled_dot_product_attention_batch(self, Q, K_s, V_s):    # scaled dot product attention: implementation style like tensor2tensor from google
        ...
        """
        # 1. split Q,K,V
        #K_s=tf.layers.dense(K_s,self.d_model) # transform K_s, while keep as shape. TODO add 2018.10.21. so that Q and K shoud be not the same.
        Q_heads = tf.stack(tf.split(Q,self.h,axis=2),axis=1)                    # [batch,h,sequence_length,d_k]
        K_heads = tf.stack(tf.split(K_s, self.h, axis=2), axis=1)               # [batch,h,sequence_length,d_k]
        ...
        if self.mask is not None:
            ...
            #dot_product:(128, 8, 6, 6);mask_expand:(1, 1, 6, 6)
...
def multi_head_attention_for_sentence_vectorized(layer_number):
    ...
    with tf.variable_scope(f"query_at_each_sentence{str(layer_number)}"):
        ...
        multi_head_attention_class = MultiHeadAttention(Q, K_s, V_s, d_model, d_k, d_v, sequence_length, h,typee='decoder',mask=mask)
